...
class ChessArmControl: public rclcpp::Node{
  public:
    ChessArmControl(): Node("chess_arm_control")
    {
      subscription_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
        "chess_pos", 20, std::bind(&ChessArmControl::trajectory_run, this, _1));
    }
  private:
    void trajectory_run(const std_msgs::msg::Float32MultiArray & a_msg) const{

      std::cout<<"THIS RANNNN MFERRRR";
      ...
      if(a_msg.data[0] == -100.0){
        ...
        std::vector<double> joint_group_positions = {a_msg.data[0], a_msg.data[1], a_msg.data[2], a_msg.data[3]};
        ...
        if (success) {
          ...
          std::this_thread::sleep_for(std::chrono::seconds(1));  // Wait for 2 seconds after execution
        } else {
...
int main(int argc, char * argv[])
{
  ...
  */
  ...
  rclcpp::spin(std::make_shared<ChessArmControl>());
  rclcpp::shutdown();
  return 0;
}
